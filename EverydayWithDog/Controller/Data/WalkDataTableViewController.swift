...
class WalkDataTableViewController: UITableViewController,SegementSlideContentScrollViewDelegate {
  ...
  override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //いつ食べたか時刻を表示
        cell.textLabel?.text = self.walkInfoArray[indexPath.row].dayString
        cell.detailTextLabel?.text = self.walkInfoArray[indexPath.row].distanceString
        return cell
    }
...
//    }
    ...
    func loadWalkInfo() {
        let fetchWalkInfo = Firestore.firestore().collection("user").document(uid!).collection("dogList").document(self.dogID!).collection("walkInfomation")
               fetchWalkInfo.getDocuments() { (querySnapshot, err) in
                   if let err = err {
                       print("Error getting documents: \(err)")
                   } else {
                    //[]でwalkInfoArrayを0にしておかないとセルが増えていくため、初期化する
                    self.walkInfoArray = []
                       for document in querySnapshot!.documents {
                           if let walkData = document.data() as? [String: Any] {
                                let startTimeString = walkData["startTimeString"] as? String
                            
                            let endTimeString = walkData["endTimeString"] as? String
                               
                                let walkPlaceString = walkData["walkPlaceString"] as? String
                               
                                let walkDistanceStirng = walkData["walkDistanceStirng"] as? String
                               
                            self.walkInfoArray.append(WalkInfo(dayString: startTimeString!, endTimeString: endTimeString!, walkPlaceString: walkPlaceString!, distanceString: walkDistanceStirng! + "km"))
                           }
